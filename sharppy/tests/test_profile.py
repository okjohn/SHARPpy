...
sounding2 = """
 1000.00,    133.00,  -9999.00,  -9999.00,  -9999.00,  -9999.00
  ...
  971.00,    400.17,     21.20,     14.20,  -9999.00,  -9999.00
  946.89,    610.00,     18.79,     12.65,    165.00,     19.00
  943.00,    644.39,    -9999.,    -9999.,  -9999.00,  -9999.00
  925.00,    804.00,     16.80,     12.10,    165.00,     20.01
  913.06,    914.00,     15.75,     11.83,    165.00,     21.00
  880.76,   1219.00,     12.85,     11.07,    165.00,     22.01
  878.00,   1245.58,     12.60,     11.00,  -9999.00,  -9999.00
...
for sline in slines:
    parts = sline.split(',')
    pres2.append(float(parts[0]))
    hght2.append(float(parts[1].strip()))
    tmpc2.append(float(parts[2].strip()))
    dwpc2.append(float(parts[3].strip()))
    wdir2.append(float(parts[4].strip()))
    wspd2.append(float(parts[5].strip()))
pres2 = ma.asarray(pres2)
hght2 = ma.asarray(hght2)
tmpc2 = ma.asarray(tmpc2)
dwpc2 = ma.asarray(dwpc2)
wdir2 = ma.asarray(wdir2)
wspd2 = ma.asarray(wspd2)
prof2 = Profile(pres=pres2, hght=hght2, tmpc=tmpc2, dwpc=dwpc2,
                wdir=wdir2, wspd=wspd2)
